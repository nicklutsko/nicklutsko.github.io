...
$$ 
where $\Gamma_d$ is the dry adiabatic lapse rate. Since $\Gamma_e - \Gamma_m$ is greater than zero, and $w_{cl}$ is positive we want $w_{cl}$ to be large so that $T_{cl}$ increases rapidly. Conversely, $\Gamma_e - \Gamma_d$ is less than zero, as is $w_c$, and so $w_c$ must be small for $T_c$ to increase slowly.</p>
