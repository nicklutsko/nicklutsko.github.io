...
<p>To understand the FAT hypothese, start by assuming the radiative cooling profile $(Q_c)$ is roughly constant up to some height just below the tropopause, at which it quickly goes to zero. In this region the subsidence must also be small, from the balance at the top of the page, and so the rising air must spread out at this height, forming the anvil $($the rising air has to go somewhere$)$. Another way of stating the FAT then is to say that $dQ_c/dz$ is largest at a fixed temperature. </p> 
