...
<p>A cloud-based emergent constraint that I've played around with is a "regional" constraint, which uses local regression co-efficients (\(\alpha_{loc}\)) between net or SW CRE and surface temperatures, binned into 5&#176; by 5&#176; boxes (the size of the boxes doesn't really matter). That is, for a box \(i\), \(\alpha_{loc, i} = \delta \)net CRE \(/ \delta T_s\), estimated using monthly or annual-mean data. These local regression co-efficients, for each box, can then be correlated across models with the models' (global) climate sensitivities.</p>
...
<p>A number of "hot-spots" show-up, mostly on the edges of subtropical stratocumulus regions, with the most prominent hot-spot in the northeast Pacific, roughly centered over Hawaii. Combining surface temperatures from the ERA-Interim reanalysis and net CRE data from CERES, I have a rough emergent constraint on ECS based on the average value of monthly \(\alpha_{loc}\) in the region outlined by the red boxes.</p>
...
Apart from some hints in the Southern Ocean, there don't seem to be any hot-spot regions in which local cloud fluctuations on monthly or annual time-scales tell us something about climate sensitivity. My guess is the other cloud-based constraints capture similar intermodel differences to the regional constraint, and also do worse in CMIP6 than in CMIP5.

Why wouldn't cloud-based emergent constraints work in CMIP6? <a href="https://agupubs.onlinelibrary.wiley.com/doi/full/10.1029/2019GL085782">Zelinka et al.</a> showed that (in the ensemble-mean) the high ECS values in CMIP6 are mostly due to large cloud feedbacks over the Southern Ocean. Whereas in CMIP5 (and also in CMIP3), most of the uncertainty in the cloud feedback came from subtropical descent regions. So one possibility is that CMIP6 has something like three classes of models: low sensitivity models, "CMIP5-like" models with high sensitivities driven by subtropical clouds, and a new set of high sensitivity models driven by Southern Ocean cloud feedbacks (see also <a href="https://twitter.com/mzelinka/status/1256008091029499904">this plot</a> Mark posted on Twitter). Emergent constraints based on the variability of clouds in regions of subtropical descent would then fail, as would constraints based on Southern Ocean clouds. The only hope for a cloud-based emergent constraint would be if there were correlations between Southern Ocean clouds and subtropical clouds, or if we could rule out one group of high sensitivity models.
